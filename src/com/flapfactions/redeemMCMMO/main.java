		...
		if(commandLabel.equalsIgnoreCase("addcredits")) {
			if(args.length <= 1) {
				...
				if(target == null) {
					...
					String targetPlayer = target.getUniqueId().toString();
					...
					if(targetName != null) {
						...
						saveConfig();
						sender.sendMessage(ChatColor.GREEN + "You have given " + ChatColor.GOLD + args[0] + ", " + amount + ChatColor.GREEN + " MCMMO credits.");
			...
			if(args.length <= 1) {
				...
				if(target == null) {
					...
					String targetName = getConfig().getString(target.getUniqueId().toString());
					try {
						Integer.parseInt(args[1]);
					}
					catch(NumberFormatException ex) {
					...
					if(targetName != null) {
						int oldAmount = getConfig().getInt(target.getUniqueId().toString() + ".credits");
						if(amount <= oldAmount) {
							int newAmount = oldAmount-amount;
							getConfig().set(target.getUniqueId().toString() + ".credits", newAmount);
							saveConfig();
							...
							sender.sendMessage(ChatColor.YELLOW + targetPlayer + ChatColor.RED + " already has 0 credits!");
							return true;
						}
					} else {
						sender.sendMessage(ChatColor.YELLOW + targetPlayer + ChatColor.RED + " already has 0 credits!");
			...
			if(args.length <= 1) {
				...
				int amount = Integer.parseInt(args[1]);
				int oldamount = getConfig().getInt(player.getUniqueId().toString() + ".credits");
				if(oldamount < amount) {
				...
				int newamount = oldamount-amount;
				getConfig().set(player.getUniqueId().toString() + ".credits", newamount);
				saveConfig();
			...
			if(sender instanceof Player) {
				...
				if(getConfig().getBoolean("vault") == true) {
					if(args.length == 0) {
						...
						if(balance >= cost) {
							String playerName1 = player.getUniqueId().toString();
